...
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[++index]||imagesCollection[0];$(".lightboxImage").attr("src",$(next).attr("src"))},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${
        lightboxId ? lightboxId : "galleryLightbox"
            ...
            </div>`)},showItemTags(gallery,position,tags){var tagItems='<li class="nav-item" role="option" aria-selected="true"><span class="nav-link active active-tag"  data-images-toggle="all">Tous</span></li>';$.each(tags,function(index,value){tagItems+=`<li class="nav-item active" role="option" aria-selected="false">
                <span class="nav-link" data-images-toggle="${value}">${value}</span></li>`});var tagsRow=`<ul class="my-4 tags-bar nav nav-pills" role="listbox">${tagItems}</ul>`;if(position==="bottom"){gallery.append(tagsRow)}else if(position==="top"){gallery.prepend(tagsRow)}else{console.error(`Unknown tags position: ${position}`)}},filterByTag(){var that=$(this)
if(that.hasClass("active")){return}
$('.nav-item > .nav-link').each(function(i){$(this).removeClass("active");that.addClass("active")})
